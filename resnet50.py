...
# initial model
# """
# model = models.resnet50(pretrained=True)  # 选择是否预训练
# # for param in model.parameters():
# #     param.requires_grad = False
#
#
# num_ftrs=model.fc.in_features
# model.fc = nn.Linear(num_ftrs, num_classes)  # 按类分类
...
#         best_prec = accuracy
#         torch.save(model, os.path.join(save_path, str(num_classes) + 'cbest.pth'))
#         print("model saved\n")
#     else:
...
# test_result_file.write('Test loss: {:.4f}\tTest accuracy: {:.2f}%'.format(test_loss / 12, correct_pred.item() / 120 * 100))
# test_cfg_file = open(test_result_path + "/resnet50_origin.txt", 'w')
# test_cfg_file.close()
...
def train(save_path,opt):
    ...
    model = models.resnet50(pretrained=pretrained)  # 选择是否预训练
    ...
    for epoch in range(num_epochs):
        if epoch==10:
            optimizer = torch.optim.Adam(model.parameters(), lr=opt.learning_rate/2)
        if epoch==30:
            optimizer = torch.optim.Adam(model.parameters(), lr=opt.learning_rate/10)
        start = time.perf_counter()
        model.train()
        running_loss = 0.0
        correct_pred = 0
